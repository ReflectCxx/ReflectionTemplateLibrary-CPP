...
namespace rtl::erase
{
    ...
    struct aware_method<const record_t, return_t, signature_ts...> : public erased_method<record_t, signature_ts...>
    {
        ...
        aware_method()
        {
            base_t::v_hop = void_hop;
            base_t::r_hop = return_hop;
        }
        ...
        ForceInline static std::any return_hop(const base_t* p_this, const record_t& p_target, signature_ts&&...params) noexcept
        {
            if constexpr (!std::is_void_v<return_t>)
            {
                auto this_p = static_cast<const this_t*>(p_this);

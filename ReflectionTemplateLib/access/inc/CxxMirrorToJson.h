...
namespace rtl {
	...
	{
		static const std::string toJson(const detail::FunctorId& pFunctorId)
		{
			std::stringstream sout;
			sout << "{\"containerId\": \"" << std::to_string(pFunctorId.getSignatureId()) << "\",";
			sout << "\"index\": \"" << std::to_string(pFunctorId.getIndex()) << "\",";
			if (pFunctorId.getRecordId() != detail::TypeId<>::None) {
		...
		template<unsigned int N>
		static const std::string toJson(const CxxMirror<N>& pCxxMirror)
		{
			std::stringstream sout;
			sout << "[";
		...
		template<unsigned int N>
		static void dump(const CxxMirror<N>& pCxxMirror, const std::string& pFilePathStr)
		{

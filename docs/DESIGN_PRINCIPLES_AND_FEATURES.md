...
RTL employs a two-phase invocation model. Metadata queries return lightweight descriptors such as `rtl::Function` and `rtl::Method`, which must be explicitly **materialized** into functors – callable entities, by specifying the expected signature.
...
By requiring explicit materialization, RTL produces lightweight, reusable functors that behave like ordinary value-type objects and can be stored in standard containers.
